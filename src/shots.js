...
export function buildScene(){
  ...
  O.layers=[['home_nav','Navigation'],['home_hero','Hero'],['stats_band','Chiffres'],
    ...
    .map(([k,tag])=>{const o=add(card(1060,k,'layer'));const t=document.createElement('div');
...
function coldOpen(t,g){
  ...
  const L=O.logo;
  L._mask.style.height=(E.outQuint(clamp(inv(.2,2.0,lt)))*(L._img.offsetHeight||248)+2)+'px';
...
function heroReveal(t,g){
  ...
  place(O.win,{x:lerp(-30,0,k2),y,z,rx:lerp(12,3.4,k2),ry:lerp(-25,-6.5,k2),o:G,s:1});
  ...
  caption(t,{eyebrow:B.eyebrow,text:'the-shift.ai',t0:m.t0+1.4,t1:m.t1-.3});
}
function promise(t,g){
  ...
  place(O.hero,{x:lerp(70,-10,k),y:lerp(30,-52,k),z:lerp(zStart,zEnd,k),
    rx:lerp(9,2.2,k),ry:lerp(15,3,k),o:G,s:1});
  ...
  g.gridK=.5;
  caption(t,{eyebrow:'La promesse, mot pour mot',text:'Ou elle ne sera plus.',t0:m.t0+1.3,t1:m.t1-.3});
}
function stats(t,g){
  const m=M.stats, lt=t-m.t0, G=gate();
  const travel=E.inOutQuint(clamp(inv(.2,6.0,lt)));
  O.stats.forEach((s,i)=>{
    const d=.12+i*.09, kk=clamp(inv(d,d+1.2,lt)), k=E.outQuint(kk);
    ...
    const c0=d+.3;
    s._v.textContent=count(lt,c0,c0+1.15,0,s._spec.v)+s._spec.suf;
    s._bar.style.width=(E.outExpo(clamp(inv(c0,c0+1.4,lt)))*100).toFixed(1)+'%';
  });
  ...
  g.gridK=.45;
  caption(t,{eyebrow:'Les chiffres de la page',text:'94 % n’en tirent aucune valeur.',t0:m.t0+2.4,t1:m.t1-.3});
}
function exploded(t,g){
  ...
  const sep=lerp(.16,1,openK)*(1-clamp(closeK)*.84)+.16*clamp(closeK);  // jamais un empilement exact
  const orbit=E.inOutQuint(clamp(inv(.3,5.4,lt)));
  const N=O.layers.length;
  const zc=zfit(1060,1060*ar('home_hero'),.66,.62);
  O.layers.forEach((L,i)=>{
    const c=i-(N-1)/2;
    const z=zc + c*200*sep, y=c*92*sep-48, x=c*22*sep;
    const d=Math.abs(c*200*sep)*.0055;
    place(L,{x,y,z,rx:lerp(3,15,sep),ry:lerp(0,-3.2,sep),o:G,s:lerp(1,.88,sep*.45),blur:d});
...
function services(t,g){
  ...
  const base={x:lerp(-24,10,k),y:lerp(34,-46,k),z:lerp(zStart,zEnd,k),rx:lerp(8,2,k),ry:lerp(-12,3.5,k)};
  ...
  g.gridK=.42;
  caption(t,{eyebrow:'Ce qu’on déploie',text:'Six modules, connectables.',t0:m.t0+.5,t1:m.t1-.3});
}
function donna(t,g){
  ...
  O.donna._shots.s.style.transform=`translateY(${(-scrollK*(full-view)).toFixed(1)}px)`;
  const zEnd=zfit(1180,782,.78,.74), zStart=zEnd-300;
  place(O.donna,{x:lerp(-90,30,k),y:lerp(10,-52,k),z:lerp(zStart,zEnd,k),rx:lerp(7,1.8,k),ry:lerp(15,-4.5,k),o:G});
  ...
  caption(t,{eyebrow:'Produit',text:'Donna — employée IA de direction.',t0:m.t0+.5,t1:m.t1-.3});
}
function calc(t,g){
  ...
  const sw=E.inOut(clamp(inv(drag0+.1,drag1-.05,lt)));
  const base={x:lerp(-16,14,k),y:lerp(20,-56,k),z:lerp(zStart,zEnd,k),rx:lerp(8,2,k),ry:lerp(-11,3.5,k)};
  ...
  g.gridK=.42;
  caption(t,{eyebrow:'Calculateur d’économies du site',text:'92 400 € → 316 800 €',t0:m.t0+.5,t1:m.t1-.3});
}
function devices(t,g){
  ...
  shotOp(O.win,'home',1); shotOp(O.win,'mfa',0); shotOp(O.win,'produits',0);
  O.win._path.textContent='';
  place(O.win,{x:lerp(-60,-18,k)+(1-en(0))*54,y:lerp(-90,-108,k)+(1-en(0))*30,z:lerp(-430,-380,k)-(1-en(0))*90,rx:3.4,ry:lerp(15,8,k),o:G,s:1});
  place(O.laptop,{x:lerp(432,464,k)+(1-en(1))*46,y:lerp(112,96,k)+(1-en(1))*34,z:lerp(-140,-90,k)-(1-en(1))*90,rx:lerp(6,3.5,k),ry:lerp(-19,-12,k),o:G,s:1});
  ...
  place(O.phone,{x:lerp(-590,-560,k)-(1-en(2))*46,y:lerp(112,96,k)+(1-en(2))*34,z:lerp(20,70,k)-(1-en(2))*90,rx:lerp(4,2,k),ry:lerp(21,13,k),o:G,s:1});
  place(O.deskShadow,{x:452,y:392,z:-190,o:G*.3});
  applyCam({x:lerp(60,-40,k),y:lerp(-14,10,k),z:0,ry:lerp(-5.5,4.5,k),rx:lerp(-1,2,k),dof:0});
...
function proof(t,g){
  ...
  const zEnd=zfit(1280,844,.76,.66), zStart=zEnd-240;
  place(O.win,{x:lerp(-16,8,k),y:lerp(-58,-96,k),z:lerp(zStart,zEnd,k),rx:lerp(8,2.6,k),ry:lerp(-11,-3,k),o:G});
  place(O.winShadow,{x:0,y:lerp(346,308,k),z:lerp(zStart-140,zEnd-150,k),o:G*.34,s:.92});
  O.clients.forEach((c,i)=>{
    const d=.7+i*.11, kk=E.outQuint(clamp(inv(d,d+.8,lt)));
    place(c,{x:(i-2)*312,y:lerp(392,372,k),z:lerp(-60,-20,k),ry:lerp(-3,1.5,k),o:G*kk*.6,s:lerp(.92,1,kk)});
  });
  applyCam({x:lerp(14,-8,k),y:lerp(16,-10,k),z:0,ry:lerp(1.8,-1.2,k),rx:lerp(-.8,1.6,k),dof:0});
  cursorHide(); g.gridK=.5;
  caption(t,{eyebrow:'Référence en production',text:'MFA — 14 marques, un agent.',t0:m.t0+.4,t1:m.t1-.3});
}
function endcard(t,g){
  ...
  place(L,{x:0,y:lerp(-210,-196,k),z:lerp(-140,-20,k),o:G,s:lerp(.92,1,k)});
  place(O.endCta,{x:0,y:lerp(112,88,E.spring(clamp(inv(.6,1.9,lt)),2.4,5)),z:lerp(-60,20,k),
    o:G*E.outQuint(clamp(inv(.55,1.4,lt)))});
  place(O.endUrl,{x:0,y:lerp(214,196,k),z:lerp(-60,20,k),o:G*E.outQuint(clamp(inv(.9,1.8,lt)))*.9});
  place(O.endProd,{x:0,y:lerp(-92,-104,k),z:lerp(-60,20,k),o:G*E.outQuint(clamp(inv(1.1,2.0,lt)))*.62});
  place(O.endFoot,{x:0,y:lerp(300,286,k),z:lerp(-60,20,k),o:G*E.outQuint(clamp(inv(1.5,2.4,lt)))*.5});
...
export function render(t){
  ...
  hideAll();
  const glob=Math.min(E.out(clamp(inv(0,.45,t))),1-E.in(clamp(inv(TOTAL-.6,TOTAL,t))));
